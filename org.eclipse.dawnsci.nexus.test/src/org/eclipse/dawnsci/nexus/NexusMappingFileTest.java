...
public class NexusMappingFileTest extends AbstractNexusFileTestBase {
	...
	private void addDataBlock(NXentry entry) {
		...
		data.setAttribute(null, NexusConstants.DATA_SIGNAL, NexusConstants.DATA_DATA);
		data.setAttribute(null, NexusConstants.DATA_AXES, DatasetFactory.createFromObject(new String[] {"x_stage_set", "y_stage_set", "t_stage_set", "energy"}));
	...
	private void addSample(NXentry entry) {
		...
		t_stage.setDataset(DatasetFactory.ones(ShortDataset.class, X_SHAPE, Y_SHAPE, T_SHAPE));
		...
		x_stage.setDataset(DatasetFactory.ones(ShortDataset.class, X_SHAPE, Y_SHAPE, T_SHAPE));
		...
		y_stage.setDataset(DatasetFactory.ones(ShortDataset.class, X_SHAPE, Y_SHAPE, T_SHAPE));
		...
		sample.setDataset("t_stage_set", DatasetFactory.createRange(ShortDataset.class, T_SHAPE));
		sample.setDataset("x_stage_set", DatasetFactory.createRange(ShortDataset.class, X_SHAPE));
		sample.setDataset("y_stage_set", DatasetFactory.createRange(ShortDataset.class, Y_SHAPE));
	...
	private void addItMonitor(NXinstrument instrument) {
		...
		monitor.setDataset("data", DatasetFactory.ones(ShortDataset.class, X_SHAPE, Y_SHAPE, T_SHAPE));
	...
	private void addMonitorArmTransform(NXmonitor monitor) {
		if (monitorArmTransformCache == null) {
			...
			monitor_arm.setDataset(DatasetFactory.ones(ShortDataset.class, X_SHAPE, Y_SHAPE, T_SHAPE));
	...
	private void addDiffractionDetector(NXinstrument instrument) {
		...
		detector.setAttribute(null, NexusConstants.DATA_SIGNAL, NexusConstants.DATA_DATA);	
		detector.setX_pixel_sizeScalar(1.0);
		detector.setAttribute(NXdetector.NX_X_PIXEL_SIZE, NexusConstants.UNITS, "um");
		detector.setY_pixel_sizeScalar(1.0);
		detector.setAttribute(NXdetector.NX_Y_PIXEL_SIZE, NexusConstants.UNITS, "um");

		detector.setDataset("data", DatasetFactory.ones(ShortDataset.class, X_SHAPE, Y_SHAPE, T_SHAPE, X_PIXEL_SHAPE, Y_PIXEL_SHAPE));
	...
	private void addFluorecenceDetector(NXinstrument instrument) {
		NXdetector detector = NexusNodeFactory.createNXdetector();
		detector.setAttribute(null, NexusConstants.DATA_SIGNAL, NexusConstants.DATA_DATA);	
		detector.setAttribute(null, NexusConstants.DATA_AXES, DatasetFactory.createFromObject(new String[] {".", ".", ".", "energy"}));
		detector.setAttribute(null, "energy_indices", "3");

		detector.setDataset("data", DatasetFactory.ones(ShortDataset.class, X_SHAPE, Y_SHAPE, T_SHAPE, ENERGY_SHAPE));
		detector.setAttribute("data", "interpretation", "spectrum");

		detector.setDataset("energy", DatasetFactory.createRange(ShortDataset.class, ENERGY_SHAPE));
		detector.setAttribute("energy", NexusConstants.UNITS, "keV");
	...
	private void addDetectorArmTransform(NXdetector detector) {
		if (detectorArmTransformCache == null) {
			...
			detector_arm.setDataset(DatasetFactory.ones(ShortDataset.class, X_SHAPE, Y_SHAPE, T_SHAPE));
	...
	private void addMicroscopeData(NXentry entry) {
		if (microDataCache == null) {
			...
			data.setAttribute(null, NexusConstants.DATA_SIGNAL, NexusConstants.DATA_DATA);	
			data.setAttribute(null, NexusConstants.DATA_AXES, DatasetFactory.createFromObject(new String[] {".", "image_x", "image_y"}));
			data.setAttribute(null, "image_x_indices", "1");
			data.setAttribute(null, "image_y_indices", "2");

			data.setDataset("data", DatasetFactory.ones(ShortDataset.class, 4, MICRO_X_SHAPE, MICRO_Y_SHAPE));
			data.setAttribute("data", "interpretation", "rgba-image");	
			
			data.setDataset("image_x", DatasetFactory.createRange(ShortDataset.class, MICRO_X_SHAPE));
			data.setAttribute("image_x", NexusConstants.UNITS, "mm");

			data.setDataset("image_y", DatasetFactory.createRange(ShortDataset.class, MICRO_Y_SHAPE));
			data.setAttribute("image_y", NexusConstants.UNITS, "mm");
