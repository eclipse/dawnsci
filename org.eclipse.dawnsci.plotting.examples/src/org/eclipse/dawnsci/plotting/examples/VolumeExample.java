...
public class VolumeExample extends PlotExample {
	...
	protected void createExampleContent(Composite parent) {
		...
		try {
			...
			system.createPlotPart(parent, "Volume Example", getViewSite().getActionBars(), PlotType.VOLUME, this);
			system.setPlotType(PlotType.VOLUME);
			File loc = new File(BundleUtils.getBundleLocation(Activator.PLUGIN_ID), getFileName());
			...
			final IVolumeRenderTrace volume = system.createVolumeRenderTrace("Volume1");
			
			volume.setData(
					lazyDataset.getShape(), 
					lazyDataset.getSlice(new int[]{0,0,0}, lazyDataset.getShape(), new int[]{2,2,2}), 
					0.42, 
					0.61, 
					new double[]{0.2,5.0}, 
